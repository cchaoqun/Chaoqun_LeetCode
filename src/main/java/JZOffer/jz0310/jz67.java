...
public class jz67 {
    ...
    public int strToInt(String str) {
        ...
        int index = 0;
        int res = 0;
        while(index<arr.length && isSpac(arr[index])){
            ++index;
        }
        //判断是否有符号
        if(index<arr.length && isOper(arr[index])){
            if(arr[index]=='-'){
                sign *= -1;
            }
            ++index;
        ...
        //第一个非空字符不是数字
        if(index>=arr.length || !isNum(arr[index])){
            return 0;
        }
        //判断数字字符
        while(index<arr.length && isNum(arr[index])){
            int cur = arr[index]-'0';
            if(sign==1){
                //正数
